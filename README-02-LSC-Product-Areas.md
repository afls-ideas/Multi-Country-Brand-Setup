...
| `Id IN (:allPTDAProductIds)` | Product must be territory-aligned (via PTDA records) | Missing `ProductTerritoryAvailability` record → no PTDA → product not in pool. Run alignment batch job after creating PTAs. |
